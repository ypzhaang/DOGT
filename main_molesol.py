...
def compute_metrics_zinc_molesol(pred, label, dataset):
    mae = MF.mean_absolute_error(pred, label)
    rmse = torch.sqrt(F.mse_loss(pred, label))
    pcc = MF.pearson_corrcoef(pred.view(-1), label.view(-1))
    if dataset == 'zinc-mini':
        main_metric = mae
    elif dataset == 'molesol':
        main_metric = rmse
    else:
        raise ValueError("Unsupported dataset for metrics")
    return {
        'main': main_metric.item(),
        'mae': mae.item(),
        'rmse': rmse.item(),
        'pcc': pcc.item()
    }
...
def evaluate(model, loader, eval_func, device, args):
    ...
    elif args.dataset in ['zinc-mini', 'molesol']:
        metrics = compute_metrics_zinc_molesol(y_pred, y_true, args.dataset)
    else:
        metrics = {'main': main_score.item() if hasattr(main_score, 'item') else main_score}
...
def save_top_worst_epochs(results_dir, epoch_score_list, dataset, run, K=10):
    metric_name = 'mae' if dataset == 'zinc-mini' else 'rmse'
    reverse_order = metric_name == 'mae'
    sorted_list = sorted(epoch_score_list, key=lambda x: x[1][metric_name], reverse=reverse_order)

    df_top = pd.DataFrame([(e, m['mae'], m['rmse'], m['pcc']) for e, m in sorted_list[:K]],
                          columns=["epoch", "mae", "rmse", "pcc"])
    df_worst = pd.DataFrame([(e, m['mae'], m['rmse'], m['pcc']) for e, m in sorted_list[-K:]],
                            columns=["epoch", "mae", "rmse", "pcc"])
...
for run in range(args.runs):
    results_dir = f"/home/dell/sx/sgformer/SGFormer/large/new/results_molesol"
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.eval_step == 0:
                ...
                # 保存 test main score
                epoch_score_list.append((epoch, test_result))
        ...
        # df_top.to_csv(os.path.join(results_dir, f"top_epochs_run{run}.csv"), index=False)
        # df_worst.to_csv(os.path.join(results_dir, f"worst_epochs_run{run}.csv"), index=False)
        save_top_worst_epochs(results_dir, epoch_score_list, args.dataset, run)
