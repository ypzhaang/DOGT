...
if args.dataset == 'molhiv':
    ...
    dataset, train_loader, valid_loader, test_loader, num_features, num_classes = load_mnist_dataset(args)
else:
...
mask_dir = "/home/dell/sx/sgformer/SGFormer/large/new/mnist_mask"
# ------------------- 主训练循环 -------------------
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            elif args.dataset in ['fer123', 'fer123happyandsad', 'fer123angryandfear','mnist']:
                label = batch.y.long()
                loss = criterion(out, label)
            else:
                label = batch.y.float()
        ...
        if epoch % args.eval_step == 0:
            ...
            if args.dataset.startswith('fer'):
                H = W = 48          # FER-2013 / FER-Plus
            elif args.dataset == 'mnist':
                H = W = 28          # MNIST
            else:
                # 若其他数据集确实是正方形图，可用节点数开方推
                H = W = int(node_indices.numel() ** 0.5)
            ...
            for graph_id in range(num_graphs):
                ...
                if args.dataset.startswith('fer'):
                    expected_nodes = 48 * 48       # 2304
                elif args.dataset == 'mnist':
                    expected_nodes = 28 * 28       # 784
                else:
                    expected_nodes = None          # 或者其他尺寸
                ...
                # 构建图像
                pixels = batch.x[node_indices].view(H, W).detach().cpu().numpy()
