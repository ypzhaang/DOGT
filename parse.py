from gnns import *
from ours_new import *
from ours_new import SGFormer
from ours_withhyper import AGGT
...
def parse_method(args, c, d, device):
    ...
    elif args.method == 'AGGT':
        model = AGGT(d, args.hidden_channels, c, graph_weight=args.graph_weight, aggregate=args.aggregate,
                    trans_num_layers=args.trans_num_layers, trans_dropout=args.trans_dropout, trans_num_heads=args.trans_num_heads, trans_use_bn=args.trans_use_bn, trans_use_residual=args.trans_use_residual, trans_use_weight=args.trans_use_weight, trans_use_act=args.trans_use_act,
                     gnn_num_layers=args.gnn_num_layers, gnn_dropout=args.gnn_dropout, gnn_use_bn=args.gnn_use_bn, gnn_use_residual=args.gnn_use_residual, gnn_use_weight=args.gnn_use_weight, gnn_use_init=args.gnn_use_init, gnn_use_act=args.gnn_use_act, gnn_for_origin_num_layers = args.gnn_for_origin_num_layers, 
                     n_hypers = args.n_hypers, hyper_dropout = args.hyper_dropout, hyper_weight = args.hyper_weight
                     ).to(device)
    else:
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--data_dir', type=str, default='/home/dell/sx/sgformer/SGFormer/data')
    ...
    parser.add_argument('--gnn_for_origin_weight_decay', type=float, default=1e-3)
    parser.add_argument('--gnn_for_origin_dropout', type=float, default=0.0)
    parser.add_argument('--gnn_for_origin_num_layers', type=int, default=2, help='number of layers for origin GNN')
    ...
    # hyper branch
    parser.add_argument('--n_hypers', type=int, default=16)
    parser.add_argument('--hyper_weight_decay', type=float, default=1e-3)
    parser.add_argument('--hyper_dropout', type=float, default=0.0)
    parser.add_argument('--hyper_diverse_rate', type=float, default=0.1)
    parser.add_argument('--hyper_weight', type=float, default=0.5)
