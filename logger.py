...
class Logger(object):
    ...
    def add_result(self, run, result_dict):
        """
        result_dict: dict of {"metric_name": value} for this epoch
        """
        assert run >= 0 and run < len(self.results)
        self.results[run].append(result_dict)
        ...
        if run is not None:
            ...
            for r in self.results:
                ...
                if mode == 'max_acc':
                    ...
                    train2 = r[r[:, 2].argmin(), 0].item()
                    test2 = r[r[:, 2].argmin(), 2].item()
